...
1. `bench-slow-trap` 分支没有 fast-trap，并且使用直接模式陷入，因此代理软中断时也要完整的存取上下文。相比之下，rustsbi-qemu 早就采用了中断向量表；
2. `bench-vectored-trap` 分支没有 fast-trap 但启用了中断向量表，因此其代理软中断时不必存取上下文，中断响应速度较快。这个版本相当于 rustsbi-qemu 的 slow-trap；
